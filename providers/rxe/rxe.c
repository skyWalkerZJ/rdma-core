...
int client_xchange_metadata_with_server(int sockfd)
{
    ...
    memset(src,0x11,1000);
	memset(src+1000,0x22,500);
    memset(dst,0x11,1000);
	memset(dst+1000,0x22,500);
...
static int client_remote_memory_ops() 
{
	...
	*/
	...
	ret = process_work_completion_events(io_completion_channel, 
			&wc, 2);
	if(ret != 2) {
		printf("we failed to get 4 work completions\n");
		return ret;
	}
